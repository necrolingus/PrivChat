...
/**
 * Server-Side Unique Phrase Generation
 * 
 * Security Design:
 * 1. Phrases are generated server-side using Node.js CSPRNG (crypto.randomBytes)
 * 2. The derived hash (deviceId / channelId) is checked against the database for uniqueness
 * 3. If a collision is detected, the phrase is silently regenerated (up to 10 attempts)
 * 4. A randomized artificial delay (50-200ms) is added to EVERY response to prevent
 *    timing-based side-channel attacks (attacker cannot distinguish first-try from regeneration)
 * 5. The raw phrase is NEVER stored server-side — only the hash is transiently checked
 */

// Dedicated rate limiter for generation endpoints (15 requests per minute)
const generationLimiter = rateLimit({
  windowMs: 1 * 60 * 1000, // 1 minute
  max: 15,
  message: { error: 'Too many phrase generation requests. Please slow down.' },
  standardHeaders: true,
  legacyHeaders: false,
});

// Dedicated rate limiter for token verification/guessing attempts (10 tries per 15 minutes)
const tokenVerifyLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 10,
  message: { error: 'Too many server token verification attempts. Please slow down and try again in 15 minutes.' },
  standardHeaders: true,
  legacyHeaders: false,
});

// BIP-39 compatible wordlist (2048 words) — must match client-side crypto.js WORDLIST exactly
const WORDLIST = [
...
/**
 * Generate a random phrase of N words using Node.js CSPRNG
 */
function generateRandomPhrase(wordCount) {
  const words = [];
  const bytes = crypto.randomBytes(wordCount * 4); // 4 bytes per word for uniform distribution
  for (let i = 0; i < wordCount; i++) {
    const index = bytes.readUInt32BE(i * 4) % WORDLIST.length;
...
/**
 * SHA-256 hex hash of a normalized string (matches client-side sha256Hex exactly)
 */
...
/**
 * Derive deviceId from 12-word phrase (matches client-side deriveDeviceId exactly)
 */
function deriveDeviceId(twelveWordPhrase) {
  const hashHex = sha256Hex(twelveWordPhrase + '_device_identity');
  return `${hashHex.substr(0, 8)}-${hashHex.substr(8, 4)}-4${hashHex.substr(13, 3)}-a${hashHex.substr(17, 3)}-${hashHex.substr(20, 12)}`;
}

/**
 * Derive channelId from 6-word phrase (matches client-side sha256Hex exactly)
 */
function deriveChannelId(sixWordPhrase) {
  return sha256Hex(sixWordPhrase);
}

/**
 * Add a randomized artificial delay to prevent timing attacks.
 * Always waits between 50ms and 200ms regardless of how long the internal logic took.
 */
function randomDelay() {
  const delayMs = 50 + Math.floor(Math.random() * 150); // 50-200ms
  return new Promise((resolve) => setTimeout(resolve, delayMs));
}

/**
 * POST /api/generate/verify-server-token
 * Dedicated endpoint to verify a server invite token without triggering phrase generation rate limits.
 */
router.post('/verify-server-token', tokenVerifyLimiter, async (req, res) => {
  try {
    const isPrivate = String(process.env.PRIVATE_SERVER || 'false').toLowerCase() === 'true';
    if (!isPrivate) {
      return res.json({ success: true, isPrivateServer: false, message: 'Server is public.' });
    }

    const { serverToken } = req.body || {};
    if (!serverToken || !String(serverToken).trim()) {
      await randomDelay();
      return res.status(400).json({ error: 'Please enter a server invite token.' });
    }

    const tokenCode = String(serverToken).trim().toUpperCase();
    ...
    if (!srvToken) {
      return res.status(403).json({ error: 'Invalid server invite token. Please check the code or ask your server admin.' });
    }

    if (srvToken.isRevoked) {
      return res.status(403).json({ error: 'This server invite token has been revoked or invalidated by the admin.' });
    }

    if (srvToken.type === 'one_time' && srvToken.usedCount >= 1) {
      return res.status(403).json({ error: 'This single-use server invite token has already been consumed.' });
    }

    return res.json({
      success: true,
      isPrivateServer: true,
      tokenType: srvToken.type,
      message: 'Server invite token verified successfully.',
    });
  } catch (error) {
    console.error('Verify server token error:', error);
    return res.status(500).json({ error: 'Internal server error while verifying server token.' });
  }
});
 ...
 * 
 * Generates a collision-safe 12-word identity phrase.
 * The server checks that the derived deviceId does not already exist in the database.
 * If collision is found, it silently regenerates without the client knowing.
 * A randomized delay is added to prevent timing attacks.
 * 
 * The raw phrase is NEVER stored server-side.
 */
router.post('/identity-phrase', generationLimiter, async (req, res) => {
  try {
    const isPrivate = String(process.env.PRIVATE_SERVER || 'false').toLowerCase() === 'true';
    const serverTokenInput = req.body.serverToken || req.headers['x-server-token'];

    if (isPrivate) {
      if (!serverTokenInput) {
        await randomDelay();
        return res.status(403).json({ error: 'Private Server: A valid server invite token is required to generate an identity on this server.' });
      }

      const tokenCode = String(serverTokenInput).trim().toUpperCase();
      const srvToken = await prisma.serverToken.findUnique({
        where: { token: tokenCode },
      });
      ...
      if (!srvToken) {
        await randomDelay();
        return res.status(403).json({ error: 'Private Server: Invalid server invite token. Please ask the server admin for an invite.' });
      }

      if (srvToken.isRevoked) {
        await randomDelay();
        return res.status(403).json({ error: 'Private Server: This server invite token has been revoked or invalidated by the admin.' });
      }
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      phrase = generateRandomPhrase(12);
      derivedDeviceId = deriveDeviceId(phrase);

      // Check if this deviceId already exists in the profiles table
      const existing = await prisma.userProfile.findUnique({
        where: { deviceId: derivedDeviceId },
      });

      if (!existing) {
        break; // Unique — no collision
      }

      // Collision detected — loop and regenerate silently
      console.warn(`[PhraseGenerator] Identity phrase collision detected (attempt ${attempt + 1}), regenerating...`);
      phrase = null;
    }

    // Always add randomized delay to prevent timing side-channel
    await randomDelay();

    if (!phrase) {
      return res.status(500).json({ error: 'Unable to generate a unique identity phrase. Please try again.' });
    }

    return res.json({ success: true, phrase });
  } catch (error) {
    // Still add delay on error path to prevent timing leak
    await randomDelay();
    console.error('Error generating identity phrase:', error);
    return res.status(500).json({ error: 'Internal server error during phrase generation' });
  }
});
 ...
 * 
 * Generates a collision-safe 6-word channel key.
 * The server checks that the derived channelId does not already exist in the database.
 * If collision is found, it silently regenerates without the client knowing.
 * A randomized delay is added to prevent timing attacks.
 * 
 * The raw phrase is NEVER stored server-side.
 */
router.post('/channel-phrase', generationLimiter, async (req, res) => {
  try {
    const MAX_ATTEMPTS = 10;
    let phrase = null;
    let derivedChannelId = null;

    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      phrase = generateRandomPhrase(6);
      derivedChannelId = deriveChannelId(phrase);

      // Check if this channelId already exists
      const existing = await prisma.channel.findUnique({
        where: { channelId: derivedChannelId },
      });
      ...
      // Collision detected — loop and regenerate silently
      console.warn(`[PhraseGenerator] Channel phrase collision detected (attempt ${attempt + 1}), regenerating...`);
      phrase = null;
    }
    ...
    if (!phrase) {
      return res.status(500).json({ error: 'Unable to generate a unique channel phrase. Please try again.' });
    }
    ...
    // Still add delay on error path to prevent timing leak
    await randomDelay();
    console.error('Error generating channel phrase:', error);
    return res.status(500).json({ error: 'Internal server error during phrase generation' });
  }
});
